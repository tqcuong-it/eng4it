---
title: "Ngày 1: Section 1 & 2 Review — Ôn Chiến Lược + Luyện Có Giờ"
slug: "day-1-section-1-2-review-on-chien-luoc-luyen-co-gio"
date: 2023-06-19
summary: "Ôn tập hệ thống chiến lược Section 1 & 2, luyện tập có bấm giờ để chuẩn bị cho Full Test."
description: "Tổng hợp chiến lược IELTS Listening Section 1 & 2 — form filling, map labeling, multiple choice, matching + luyện tập timed practice."
tags: ["ielts", "listening", "review"]
series: ["Tự Học IELTS 6.0"]
categories: ["IELTS"]
ShowToc: true
TocOpen: true
weight: 1
draft: false
---
> 📱 **Luyện tập trên App:** [Mở App IELTS](https://ielts.eng4it.com) — Flashcard từ vựng, bài tập ngữ pháp, đọc hiểu, nghe và kiểm tra có chấm điểm tự động!

## 📋 Tổng hợp chiến lược Section 1

### Các dạng câu hỏi Section 1

| Dạng | Mô tả | Chiến lược chính |
|------|--------|-----------------|
| Form Filling | Điền thông tin vào form | Dự đoán loại thông tin (tên, số, ngày) |
| Note Completion | Điền vào ghi chú | Chú ý word limit |
| Multiple Choice | Chọn đáp án đúng | Loại trừ distractor |
| Map/Plan Labeling | Gán nhãn bản đồ | Xác định vị trí xuất phát |

### Checklist Section 1

- ✅ **Đọc kỹ đề TRƯỚC khi nghe** — bạn có 30 giây
- ✅ **Dự đoán loại đáp án** — tên người? số? địa chỉ?
- ✅ **Nghe signal words** — "That's spelled...", "The number is..."
- ✅ **Viết chính xác** — spelling sai = 0 điểm
- ✅ **Chữ in hoa** cho tên riêng

---

## 📋 Tổng hợp chiến lược Section 2

### Các dạng câu hỏi Section 2

| Dạng | Mô tả | Chiến lược chính |
|------|--------|-----------------|
| Multiple Choice | 3 lựa chọn A/B/C | Nghe kỹ, đừng chọn đáp án đầu tiên |
| Matching | Nối thông tin | Đọc cả 2 cột trước |
| Map/Plan Labeling | Gán nhãn vị trí | Theo dõi hướng (left, right, opposite) |
| Sentence Completion | Hoàn thành câu | Dự đoán từ loại cần điền |

### Checklist Section 2

- ✅ **Section 2 là ĐỘC THOẠI** — chỉ 1 người nói
- ✅ Chủ đề: du lịch, sự kiện, hướng dẫn, giới thiệu cơ sở
- ✅ **Chú ý từ chuyển hướng** — "however", "but", "actually"
- ✅ **Map labeling:** xác định điểm bắt đầu (entrance, gate) rồi đi theo hướng dẫn

---

## 🎧 Luyện tập — Cambridge IELTS 14, Test 1

Bấm play và làm bài Section 1 + 2 bên dưới:

{{< youtube R18LH4MTwpk >}}

> ⏱️ Bấm giờ: 5 phút cho Section 1, 5 phút cho Section 2. Nghe **1 lần duy nhất!**

---

### Section 1: Crime Report Form (Câu 1-10)

*Complete the form below. Write ONE WORD AND/OR A NUMBER for each answer.*

**Personal information — Louise Taylor:**

- Nationality: 1 __________
- Occupation: interior designer
- Reason for visit: business (to buy antique 2 __________)
- Current address: 3 __________ Apartments (No 15)

**Details of theft:**

- Items stolen: wallet containing approximately 4 £ __________
- Also stolen: a 5 __________
- Date of theft: 6 __________

**Possible time and place of theft:**

- Location: outside the 7 __________ at about 4 pm
- Details of suspect: some boys asked for the 8 __________ then ran off
- One had a T-shirt with a picture of a tiger, about 12, slim build with 9 __________ hair

**Crime reference number:** 10 __________

---

### Section 2: Induction Talk for New Apprentices (Câu 11-20)

**Câu 11-12: Choose TWO letters, A-E.**

Which TWO pieces of advice for the first week does the manager give?

- A) get to know colleagues
- B) learn from any mistakes
- C) ask lots of questions
- D) react positively to feedback
- E) enjoy new challenges

**Câu 13-14: Choose TWO letters, A-E.**

Which TWO things does the manager say mentors can help with?

- A) confidence-building
- B) making career plans
- C) completing difficult tasks
- D) making a weekly timetable
- E) reviewing progress

**Câu 15-20: Company policy for apprentices**

*Write the correct letter, A, B or C.*

| | Policy | A = Encouraged | B = Some restrictions | C = Against the rules |
|---|---|---|---|---|
| A | It is encouraged. | | | |
| B | There are some restrictions. | | | |
| C | It is against the rules. | | | |

| # | Aspect | Answer |
|---|--------|--------|
| 15 | Using the internet | __________ |
| 16 | Flexible working | __________ |
| 17 | Booking holidays | __________ |
| 18 | Working overtime | __________ |
| 19 | Wearing trainers | __________ |
| 20 | Bringing food to work | __________ |

---

## ✅ Đáp án

{{< spoiler "Bấm để xem đáp án" >}}

### Section 1: Crime Report Form

| Câu | Đáp án |
|-----|--------|
| 1 | Canadian |
| 2 | furniture |
| 3 | Park |
| 4 | 250 |
| 5 | phone |
| 6 | 10(th) September |
| 7 | museum |
| 8 | time |
| 9 | blond(e) |
| 10 | 87954 82361 |

### Section 2: Induction for Apprentices

| Câu | Đáp án |
|-----|--------|
| 11 & 12 | A, C |
| 13 & 14 | B, E |
| 15 | B |
| 16 | B |
| 17 | C |
| 18 | A |
| 19 | A |
| 20 | A |

{{< /spoiler >}}

---

## 📝 Tự đánh giá

Sau khi làm xong, tự chấm điểm:

| Kỹ năng | Điểm (1-5) | Ghi chú |
|---------|------------|---------|
| Form Filling | /5 | |
| Map Labeling | /5 | |
| Multiple Choice | /5 | |
| Matching | /5 | |
| Nghe số & spelling | /5 | |

> 💡 **Ghi lại điểm yếu** — bạn sẽ cần thông tin này cho Day 7 (Error Analysis)!

---

## 🔗 Tài liệu tham khảo

- 📖 [IELTS Liz — Listening Tips](https://ieltsliz.com/ielts-listening-tips/)
- 🎧 [Cambridge IELTS 18 Full Listening Test](https://www.youtube.com/watch?v=yNt4oyUM6NI)
- 📝 [Cambridge Sample Tests](https://www.ielts.org/for-test-takers/sample-test-questions)
